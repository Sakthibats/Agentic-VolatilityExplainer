...
5. **`move_assessment` is computed in code** ([`tools/price.py`](../backend/volatility_explainer/tools/price.py)),
   on two axes: relative (|change| vs. this stock's expected move, scaled by √time from annualized
   realized vol) and absolute (a stock-agnostic magnitude floor, so a chronically volatile name
   can't get an 18% drop called "typical"). Output: an `overall` level plus one plain-English flag
   per non-typical horizon (1d/1w/2w/1mo/ytd/1y).
...
7. **Only two ways to finish:** `submit_analysis` (summary + ≤4 tiles + 2–3 ranked hypotheses) or
   `flag_out_of_scope`. If the model writes prose, the loop stops and the run comes back
   `status: "incomplete"` rather than hanging.
8. **Streaming the write-up.** Every turn is streamed; while `submit_analysis` is generated, its
   `summary` field is extracted from the partial JSON and emitted as cumulative `summary` events.
9. **Server-side post-processing.** Citations are attached from the first 3 real `get_news` headlines
   with a URL. Analyst upside is re-anchored to the same price the summary quotes.
...
summary     (0..n)      → SummaryProgress         cumulative summary text — replace, don't append
...
lookups, `[orchestrator]` / `[run:TICKER]` totals. Some modules (clients, analyst, events, analytics)
...
- **The loop is pinned with a fake Anthropic client** that replays scripted responses as real
  streaming events (including chunked `input_json_delta`s), so the partial-JSON summary extraction
  runs for real.
